...
Soft+Virtual contributions for the DY process D Ubar -> W- -> l- ~nu_l up to N3LO QCD, including binning
*********************************************************************
********************************************************************* */
...
********************************************************************* */
  ...
  switch(k)
    ...
    case 2: // NNLO
      ...
      log2 = log1*log1;
      delterms = 1.0 +
	2*asopi/9.0*(-24.0 + 9*log1 + 2*constants::Pi2) +
	asopi2/6480.0*
	...
	constants::b0*logmu1*2*asopi2/9.0*(-24.0 + 9*log1 + 2*constants::Pi2);
      break;
    case 3: // N3LO
	...
	asopi3*(2*(constants::b0*constants::b0*logmu2 + constants::b1*logmu1)/9.0*(-24.0 + 9*log1 + 2*constants::Pi2) +
...
********************************************************************* */
  ...
  switch(k)
	...
	asopi2*constants::b0*logmu1*8.0/3.0*log1;
      plusterms[1] = 16*asopi/3.0 -
	4*asopi2*(41 + 25*constants::Pi2 - 3*log1 - 48*log2)/27.0 +
    ...
    case 3: // N3LO
      ...
      plusterms[0] = 8*asopi/3.0*log1 +
	asopi2/81.0*(-932 + 138*constants::Pi2 - 6*(41 + 25*constants::Pi2)*log1 + 225*log2 + 3438*constants::Zeta3) +
	8*asopi2*constants::b0*logmu1/3.0*log1 +
	asopi3/87480.0*
	...
	asopi3*(8*(constants::b0*constants::b0*logmu2 + constants::b1*logmu1)/3.0*log1
		+ 2*constants::b0*logmu1*
	...
	asopi3*(
		...
		- 8/27.0*constants::b0*logmu1*(41 + 25*constants::Pi2 - 3*log1 - 48*log2));
      plusterms[2] = 4*asopi2/9.0*(-23 + 48*log1)
	-2*asopi3/81.0*
	(5515 - 1679*constants::Pi2 + 33*(-103 + 48*constants::Pi2)*log1 + 756*log2 - 384*log3 - 19896*constants::Zeta3) +
	8*asopi3*constants::b0*logmu1/9.0*(-23 + 48*log1);
      plusterms[3] = 128*asopi2/9.0 -
	4*asopi3*(-1409 + 528*constants::Pi2 + 1264*log1 - 768*log2)/81.0 + 256*asopi3*constants::b0*logmu1/9.0;
